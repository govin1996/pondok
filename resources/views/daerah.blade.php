...
<head>
  ...
  <title>Data Daerah - Smart Boarding</title>
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Data Daerah</h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                  ...
                  <table id="daerah" class="table table-bordered table-striped" width="inherit">
                    <thead>
                      <tr>
                        <th>No</th>
                        <th>Nama Daerah</th>
...
<script type="text/javascript">
  var table = $('#daerah').DataTable({
    processing: true,
    serverSide: false,
    ajax: "{{ route('api.tampildaerah') }}",
    responsive: true,
    autoWidth: true,
    columns: [
  ...
  function addForm() {
    ...
    $('.modal-title').text('Tambah Data Daerah');
  }
  ...
  function editForm(id) {
      ...
      $('#modal-form form')[0].reset();
      $.ajax({
          url: "{{ url('/daerah') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
          success: function(data) {
              $('#modal-form').modal('show');
              $('.modal-title').text('Edit Data Daerah');
              $('#id').val(data.id);
  ...
  $(function(){
    $('#modal-form form').validator().on('submit', function (e) {
        if (!e.isDefaultPrevented()) {
            var id = $('#id').val();
            if (save_method == 'add') url = "{{ url('/daerah') }}";
            else url = "{{ url('/daerah') . '/' }}" + id;
            $.ajax({
                url: url,
                type: "POST",
