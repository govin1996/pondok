...
<head>
  ...
  <title>Data Pimpinan - Smart Boarding</title>
            ...
            <div class="col-xs-12">
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <table id="pimpinan" class="table table-bordered table-striped" width="inherit">
                    <thead>
                      <tr>
                        <th>No</th>
                        {{-- <th>Kodeapp</th> --}}
...
<script type="text/javascript">
  var table = $('#pimpinan').DataTable({
    processing: true,
    serverSide: false,
    ajax: "{{ route('api.tampilpimpinan') }}",
    responsive: true,
    autoWidth: true,
    columns: [
        {data: 'DT_RowIndex', name:'DT_RowIndex'},
        // {data: 'kodeapp' , name : 'kodeapp'},
        {data: 'id_pimpinan' , name : 'id_pimpinan' },
        {data: 'nama' , name : 'nama' },
  ...
  function addForm() {
    ...
    $('.modal-title').text('Tambah Data Pimpinan');
  }
  ...
  function editForm(id) {
      ...
      $('#modal-form form')[0].reset();
      $.ajax({
          url: "{{ url('/pimpinan') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
          success: function(data) {
              $('#modal-form').modal('show');
              $('.modal-title').text('Edit Data Pimpinan');
              $('#id').val(data.id);
              $('#nama').val(data.nama);
  ...
  $(function(){
    $('#modal-form form').validator().on('submit', function (e) {
        if (!e.isDefaultPrevented()) {
            var id = $('#id').val();
            if (save_method == 'add') url = "{{ url('/pimpinan') }}";
            else url = "{{ url('/pimpinan') . '/' }}" + id;
            $.ajax({
                url: url,
                type: "POST",
