...
<head>
  ...
  <title>Data Kamar - Smart Boarding</title>
            ...
            <div class="col-xs-12">
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <table id="kamar" class="table table-bordered table-striped" width="inherit">
                    <thead>
                      <tr>
                        <th>No</th>
                        <th>Nama Kamar</th>
...
<script type="text/javascript">
  var table = $('#kamar').DataTable({
    processing: true,
    serverSide: false,
    ajax: "{{ route('api.tampilkamar') }}",
    responsive: true,
    autoWidth: true,
    columns: [
  ...
  $(function(){
    $('#modal-form form').validator().on('submit', function (e) {
        if (!e.isDefaultPrevented()) {
            var id = $('#id').val();
            if (save_method == 'add') url = "{{ url('/kamar') }}";
            else url = "{{ url('/kamar') . '/' }}" + id;
            $.ajax({
                url: url,
                type: "POST",
